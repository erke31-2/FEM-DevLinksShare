...
const LinkInput: React.FC<LinkInputProps> = ({defaultFormData, removeAddedForm}) => {
  const handleRemoveForm = () => { removeAddedForm(defaultFormData.id) }
  const user = useOutletContext<User>();
  const {mutation} = useSocialLinkMutate(user.id, handleRemoveForm)
  const { handleSubmit, control, register } = useForm<SocialLinksData>({resolver: zodResolver(UpsertSocialLinkFormData), defaultValues: defaultFormData});
  ...
  return (
    <div className="w-full bg-cardBg px-4 py-3 rounded-xl text-secondaryColor flex flex-col gap-y-3">
      <div className="flex justify-between items-center px-1">
        ...
        <button className="text-gray-500 hover:text-slate-400 font-semibold" onClick={handleRemoveForm}>Remove</button>
      </div>
      <div>
        ...
        <ReactSelect
          ...
          value={ linkName.value ? options.find((v) => v.value === linkName.value) : null }
          onChange={(option) => linkName.onChange(option ? option?.value : null) }
          {...linkNameState}
        />
      </div>
      <div>
        ...
        <div className="flex items-center border rounded-md px-3 py-[11px] gap-x-3 border-inputBorder overflow-auto">
          ...
          <input type="text" id="url" className="cursor-pointer w-full outline-none bg-cardBg" {...register("url")}/>
        </div>
      </div>
      <div>
        ...
        <div className="flex items-center border rounded-md px-3 py-[11px] border-inputBorder overflow-auto">
          <input type="text" id="description" placeholder="@eg:Follow Me On Twitter" className="cursor-pointer w-full outline-none bg-cardBg" {...register("description")}/>
        </div>
      </div>
